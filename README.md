...
支持所有模型，包括载具、武器、饰品、普通物品和地图资源。一个压缩包里有多个模型时，不传 `--model` 会把能导入的模型全部渲染出来；同名 `_hi` / `+hi` 高模会优先使用高模版本。武器截图会检测最终 PNG 的高亮比例，过曝时自动降低棚灯重渲染；同一张贴图同时用于颜色和数据通道时会使用独立的 Blender Image，避免 Diffuse 被误设为 `Non-Color`。原生 YDR 若含非法 UTF-8 内嵌贴图名，会在导入期间使用确定性的安全名称；截图导入遇到扩展顶点签名时会按已解析的默认几何布局继续，原文件不被修改。

输入树中的 `vehshare*.ytd` 与已确认跨武器共用的 `omex_collection_*.ytd` 会自动作为共享字典加载，避免模型与贴图分处不同武器目录时退化为纯色材质。若 YTD 使用 `pack:/name.dds` 等不能直接落盘的贴图名，常规提取失败后会自动使用 CodeWalker 安全名称回退提取。
...
- 模型候选收集按单次文件遍历完成，每 1,000 个文件输出一次 `[scan]` 进度；每个模型启动时输出 `[start] 模型名 当前/总数`，上万文件不会再长时间静默。
- 渲染调度只保留与 `--workers` 数量相同的活动任务，不会一次性向线程池提交上万个 Future；旧 `_vehicle_renders/_temp` 输出目录也不会参与下一轮扫描。
- 自动提取本地 `.ytd` 和共享 `vehshare.ytd`，绑定材质贴图。
- 并发任务调用 YtdTools 时使用独立 `input/dds/tool_temp`，并通过工具级锁串行进入外部程序，避免一个进程清理另一个任务的临时 YTD。
...
- Blender 后台导入模型、绑定贴图、补齐模型缺失车轮、修正玻璃/车轮基础材质；缺失图片节点按颜色、法线和高光通道生成中性回退贴图，不再显示 Blender 洋红色缺图提示。
- 缺轮车辆会按 `wheel_lf/rf/lr/rr` 或对应 hub 锚点补齐四个轮位；同侧前后复制不镜像，跨左右时才镜像轮胎网格。
- 黑色车型使用深黑基色并保留正常镜面反射与清漆层，不再强制降成低 Specular、低 Coat 的哑光表面。
- `--workers` 多进程并发渲染。
